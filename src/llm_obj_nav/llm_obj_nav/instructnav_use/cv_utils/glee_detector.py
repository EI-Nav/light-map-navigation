...
def initialize_glee(glee_config, glee_checkpoint, device="cuda:0"):
    cfg_swin = get_cfg()
...
def visualize_segmentation(image, classes, masks):
    ...
    Args:
        ...
        classes (list): List of predicted class names.
        masks (list): List of binary masks for detected objects.
    Returns:
        np.array: Annotated image with segmentation masks.
    """
    copy_image = image.copy()
    for cls, mask in zip(classes, masks):
        if len(np.unique(mask)) != 2:
            continue
        copy_image[np.where(mask == 1)] = np.random.randint(0, 255, 3)
...
def visualize_detection(image, classes, bboxes):
    ...
    Returns:
        np.array: Annotated image with bounding boxes.
    """
    copy_image = image.copy()
    for cls, bbox in zip(classes, bboxes):
        color = np.random.randint(0, 255, 3).tolist()
        copy_image = cv2.rectangle(copy_image, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color, 2)
