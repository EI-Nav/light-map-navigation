...
class HM3D_Objnav_Agent(Node):
    ...
    def reset(self):
        ...
        self.mapper.reset(self.camera_position,self.camera_rotation) 
        # 获取user的输入，这个可以手动给
    ...
    def rotate_panoramic(self,rotate_times = 6):
        ...
        for i in range(rotate_times):
            ...
            ## todo 这里有问题，由于我给的是6个方向的全景图，机器人的相机rotation应该是要发生变换的，而比是保持不变
            self.mapper.update(self.rgb_trajectory[-1],panorama['depth'][i],self.camera_position,self.camera_rotation)
            # 获取当前的语义分割图像
    ...
    def update_trajectory(self):
        ...
        # 更新地图，输入当前的RGB图像，深度图像，相机的位姿信息
        self.mapper.update(self.rgb_trajectory[-1],self.obs['depth'],self.camera_position,self.camera_rotation)
        # 获取当前的语义分割图像
        self.segmentation_trajectory.append(self.mapper.segmentation)
        # 获取当前环境中目标检测到的物体
        self.observed_objects = self.mapper.get_appeared_objects()
    ...
    def vis_valuemap_rviz(self):
        for index, (afford, safford, hafford, cafford, gafford, oafford) in enumerate(
            ...
            self.publish_pointcloud(afford, self.affordance_publisher, frame_id="d435_0_depth_optical_frame")
            self.publish_pointcloud(safford, self.semantic_publisher, frame_id="d435_0_depth_optical_frame")
            self.publish_pointcloud(hafford, self.history_publisher, frame_id="d435_0_depth_optical_frame")
            self.publish_pointcloud(cafford, self.action_publisher, frame_id="d435_0_depth_optical_frame")
            self.publish_pointcloud(gafford, self.gpt4v_publisher, frame_id="d435_0_depth_optical_frame")
            self.publish_pointcloud(oafford, self.obstacle_publisher, frame_id="d435_0_depth_optical_frame")
    ...
    def step(self):
        ...
        # todo 添加一个Nav导航器导航的命令
        # self.get_next_action(pid_waypoint)
